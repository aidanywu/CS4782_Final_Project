...
We focused on the paper's headline result from Figure 1: a smaller aligned InstructGPT model can be preferred over a much larger unaligned GPT-3 model. Since we could not reproduce the original human-labeler study, we tested the same causal claim at small scale by comparing SFT, PPO/RLHF, and DPO versions of DistilGPT2.

## GitHub Contents

- `code/CS_5782_InstructGPT_Final_Project.ipynb`: full re-implementation notebook.
- `data/`: dataset staging directory; datasets are loaded from Hugging Face in the notebook.
- `results/`: generated result figures, including reward and DPO loss summaries.
...
The notebook fine-tunes DistilGPT2 on Stanford Alpaca instruction-response examples, trains a DistilGPT2 reward model on 5,000 Anthropic Helpful-Harmless preference pairs, runs PPO with the SFT model as policy/reference, and adds a DPO branch using UltraFeedback Binarized. Evaluation uses held-out prompts, reward-model scores, pairwise win rates, response lengths, and qualitative inspection.
...
Then open `code/CS_5782_InstructGPT_Final_Project.ipynb`, mount Google Drive if using the saved-path cells, and run the notebook sections in order: setup, SFT, reward model, PPO/RLHF, DPO, evaluation, and plot generation. The notebook saves intermediate model checkpoints to paths such as `/content/drive/MyDrive/distilgpt2-sft`, so update those paths if running locally.

## Results and Insights

Preference optimization improved reward-model scores over SFT. Average rewards were **SFT: 0.4592**, **PPO/RLHF: 0.6896**, and **DPO: 0.5965**. PPO beat SFT on 80% of prompt-level reward comparisons, while DPO beat SFT on 70%. Qualitatively, aligned outputs became more instruction-shaped, but DistilGPT2 still hallucinated, repeated text, and often improved style more than factual reliability.

See `results/reward_win_summary.pdf`, `results/dpo_loss_trace.pdf`, and `report/report.pdf` for the full discussion.
...
This work was completed by Gokulanath Mahesh Kumar and Aidan Wu for Cornell CS 5782. We thank the course staff and project reviewers for feedback on the poster and reproduction scope.
